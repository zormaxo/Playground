...
using Analision.Web.Chat.SignalR;
using Analision.Web.Common;
using Analision.Web.Swagger;
using Stripe;
...
using Analision.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Analision.Web.MultiTenancy;
using Abp.HtmlSanitizer;
using Microsoft.AspNetCore.Authentication.Cookies;
using Analision.Web.Authentication.PasswordlessLogin;
using Analision.Web.OpenIddict;
using Abp.AspNetCore.OpenIddict;
using Analision.Authorization.QrLogin;

namespace Analision.Web.Startup;
...
public class Startup
{
    ...
    public IServiceProvider ConfigureServices(IServiceCollection services)
    {
        ...
        return services.AddAbp<AnalisionWebHostModule>(options =>
        {
...
#pragma warning disable CS0162
        if (AnalisionConsts.PreventNotExistingTenantSubdomains)
        {
        ...
        if (WebConsts.SwaggerUiEnabled)
        {
            ...
            {
                options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "Analision API V1");
                options.IndexStream = () => Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream("Analision.Web.wwwroot.swagger.ui.index.html");
    ...
    private void ConfigureSwagger(IServiceCollection services)
    {
        ...
        {
            options.SwaggerDoc("v1", new OpenApiInfo() { Title = "Analision API", Version = "v1" });
            ...
            if (canShowSummaries)
            {
                ...
                var applicationXml = $"Analision.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"Analision.Web.Core.xml";
