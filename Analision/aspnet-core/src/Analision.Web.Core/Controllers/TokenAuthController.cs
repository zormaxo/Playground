...
using Analision.Web.Authentication.TwoFactor;
using Analision.Web.Models.TokenAuth;
...
using Analision.Web.Authentication.External;
...
public class TokenAuthController : AnalisionControllerBase
{
    ...
    private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
    {
        ...
        var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
            .ToString(AnalisionConsts.DateTimeOffsetFormat));
