...
export class ChatMessageComponent implements OnInit {
    ...
    readonly message = input<ChatMessageDto>(undefined);
    ...
    private setChatMessageType(): void {
        ...
        this._localStorageService.getItem(AppConsts.authorization.encrptedAuthTokenName, function (err, value) {
            ...
            if (message.message.startsWith('[image]')) {
                ...
                let image = JSON.parse(message.message.substring('[image]'.length));
                self.chatMessage =
                ...
                let file = JSON.parse(message.message.substring('[file]'.length));
                self.chatMessage =
                    AppConsts.remoteServiceBaseUrl +
                    '/Chat/GetUploadedObject?fileId=' +
                    file.id +
                ...
                let linkMessage = JSON.parse(message.message.substring('[link]'.length));
